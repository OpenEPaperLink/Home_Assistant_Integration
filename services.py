...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def get_hub():
        ...
        hub = next(iter(hass.data[DOMAIN].values()))
        
        # For BLE devices, we don't need to check online status
        if hasattr(hub, '_ble_hub') and hub._ble_hub is not None:
            return hub
        
        # For AP devices, check if online
        if not hub.online:
            raise HomeAssistantError("AP is offline. Please check your network connection and AP status.")
    ...
    async def drawcustom_service(service: ServiceCall) -> None:
        ...
        3. Queues image upload to the AP or sends directly to BLE device
        ...
        for device_id in device_ids:
            ...
            try:
                ...
                try:
                    ...
                    if hasattr(hub, '_ble_hub') and hub._ble_hub is not None:
                        # BLE device - upload directly
                        await upload_image_ble(
                            hub,
                            entity_id,
                            image_data,
    ...
    async def upload_image_ble(hub, entity_id: str, img: bytes, dither: int, ttl: int) -> None:
        """Upload image to BLE device.

        Sends an image directly to a BLE device using the BLE protocol.
        Configures display parameters such as dithering and TTL.
        ...
        Raises:
            HomeAssistantError: If upload fails
        """
        mac = entity_id.split(".")[1].upper()
        _LOGGER.debug("Preparing BLE upload for %s (MAC: %s)", entity_id, mac)
        _LOGGER.debug("Upload parameters: dither=%d, ttl=%d", dither, ttl)

        # Get BLE device instance - handle both BLE_ prefix and regular MAC format
        ble_mac = mac
        if mac.startswith("BLE_"):
            ble_mac = mac[4:]  # Remove BLE_ prefix
        ble_device = hub._ble_hub.get_device(ble_mac)
        if not ble_device:
            raise HomeAssistantError(f"BLE device {mac} not found")

        try:
            # Get device dimensions from tag type manager
            hw_type = ble_device.coordinator_data.get("hw_type")
            if hw_type:
                width, height = get_hw_dimensions(hw_type)
            else:
                # Default dimensions if hardware type not available
                width, height = 128, 296

            # Convert JPEG to device format and upload
            await ble_device.upload_image(
                width=width,
                height=height,
                multi_color=True,
                compressed=True,
            ...
            _LOGGER.info("Successfully uploaded image to BLE device %s", mac)
        except Exception as err:
            raise HomeAssistantError(f"Failed to upload image to BLE device {mac}: {str(err)}")
