...
class BLEHub:
    """Manages BLE device connections and state.

    This class handles the lifecycle of BLE devices, including:
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        _LOGGER.debug("Initializing BLEHub")
        self.hass = hass
        self.entry = entry
        self._devices: Dict[str, BLEInstance] = {}
        self._shutdown = asyncio.Event()
        self._shutdown_handler: Optional[CALLBACK_TYPE] = None
        self._unsub_callbacks: list[CALLBACK_TYPE] = []

    async def async_setup(self) -> bool:
        """Set up the BLE hub."""
        _LOGGER.debug("Setting up BLEHub")
        try:
            ...
            _LOGGER.debug("BLEHub setup completed successfully")
            return True
        except Exception as err:
            _LOGGER.error("Failed to set up BLE hub: %s", err)
            return False

    async def _handle_shutdown(self, _) -> None:
        """Handle Home Assistant shutdown event."""
        _LOGGER.debug("Processing shutdown for BLE hub")
        await self.shutdown()
        self._shutdown_handler = None

    async def shutdown(self) -> None:
        """Shutdown the BLE hub."""
        self._shutdown.set()
        for device in self._devices.values():
            await device.stop()
        self._devices.clear()

    def get_device(self, mac: str) -> Optional[BLEInstance]:
        """Get a BLE device instance by MAC address."""
        return self._devices.get(mac)

    async def add_device(self, mac: str) -> BLEInstance:
        """Add a new BLE device."""
        if mac in self._devices:
            return self._devices[mac]
        
        device = BLEInstance(mac, self.hass)
        self._devices[mac] = device
        return device

    async def remove_device(self, mac: str) -> None:
        """Remove a BLE device."""
        if device := self._devices.get(mac):
            await device.stop()
...
class Hub:
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        """
        _LOGGER.debug("Initializing Hub with entry data: %s", entry.data)
        self.hass = hass
        self.entry = entry
        ...
        # Initialize BLE hub if this is a BLE device
        self._ble_hub = None
        if "mac" in entry.data:
            _LOGGER.debug("Initializing BLE hub for device: %s", entry.data["mac"])
            # Cancel any existing WebSocket tasks
            if self._ws_task and not self._ws_task.done():
                _LOGGER.debug("Cancelling existing WebSocket task")
                self._ws_task.cancel()
                self._ws_task = None
            self._ble_hub = BLEHub(hass, entry)
            # Set online state for BLE devices immediately
            self.online = True
            async_dispatcher_send(self.hass, f"{DOMAIN}_connection_status", True)
            _LOGGER.debug("BLE hub initialized and marked as online")
    ...
    async def async_setup_initial(self) -> bool:
        ...
        """
        # For BLE devices, ensure no WebSocket task is running
        if "mac" in self.entry.data:
            if self._ws_task and not self._ws_task.done():
                _LOGGER.debug("Cancelling existing WebSocket task for BLE device")
                self._ws_task.cancel()
                try:
                    await self._ws_task
        ...
        # For AP devices, try to load initial tag data
        if "host" in self.entry.data:
            try:
                tags = await self._fetch_all_tags_from_ap()
                for tag_mac, tag_data in tags.items():
                    await self._process_tag_data(tag_mac, tag_data, True)
            except Exception as err:
                _LOGGER.error("Failed to load initial tag data: %s", err)
                return False
    ...
    async def async_start_websocket(self) -> bool:
        ...
        """
        # Skip WebSocket connection for BLE devices
        if "mac" in self.entry.data:
            _LOGGER.debug("Skipping WebSocket connection for BLE device")
            return True
        ...
        if self._ws_task and not self._ws_task.done():
            ...
            try:
                ...
                _LOGGER.error("Error cleaning up previous WebSocket task: %s", e)
        ...
        try:
            ...
            if not self.online:
                ...
                # Clean up the task if connection failed
                if self._ws_task and not self._ws_task.done():
                    self._ws_task.cancel()
                    try:
                        await self._ws_task
            ...
            _LOGGER.error("Error establishing WebSocket connection: %s", e)
            # Clean up the task on any error
            if self._ws_task and not self._ws_task.done():
                self._ws_task.cancel()
                try:
                    await self._ws_task
    ...
    async def _websocket_handler(self) -> None:
        """Handle WebSocket connection lifecycle and process messages.
         ...
         """
        # Skip WebSocket handling for BLE devices
        if "mac" in self.entry.data:
            return
    ...
    def _get_hw_type_for_device(self, hw_type: str) -> int:
        """Get the correct hardware type for a device.

        For BLE devices, extracts the last two characters of the hw_type
        to get the correct tag type ID. For AP devices, returns the hw_type
        as is.

        Args:
            hw_type: The hardware type string from the device

        Returns:
            int: The correct hardware type ID for tag type lookup
        """
        if "mac" in self.entry.data and hw_type:
            # For BLE devices, use last two characters
            try:
                return int(hw_type[-2:])
            except (ValueError, IndexError):
                _LOGGER.error("Invalid hw_type format for BLE device: %s", hw_type)
                return 0
        return hw_type
    ...
    async def _process_tag_data(self, tag_mac: str, tag_data: dict, is_initial_load: bool = False) -> bool:
        ...
        if battery_mv is not None:
            try:
                ...
                battery_level = min(100, max(0, int(percentage)))
                _LOGGER.debug("Battery calculation for %s: %.3fV -> %d%%", 
                             tag_mac, voltage_v, battery_level)
            except Exception as e:
                _LOGGER.error("Error calculating battery level for %s: %s", tag_mac, e)
        
        # Get the correct hw_type for tag type lookup
        tag_type_id = self._get_hw_type_for_device(hw_type)
        _LOGGER.debug("Device %s: Original hw_type %s, using tag_type_id %s", 
                     tag_mac, hw_type, tag_type_id)
        
        hw_string = get_hw_string(tag_type_id)
        width, height = self._tag_manager.get_hw_dimensions(tag_type_id)
    ...
    def get_tag_data(self, tag_mac: str) -> dict:
        """Get the current data for a tag.

        Returns the most recent data dictionary for the specified tag,
        including all properties and state information.

        Args:
            tag_mac: MAC address of the tag

        Returns:
            dict: Tag data dictionary, or empty dict if tag not found
        """
        if "mac" in self.entry.data and self._ble_hub:
            # For BLE devices, get data from the BLE hub
            ble_device = self._ble_hub.get_device(tag_mac)
            if ble_device and ble_device.coordinator_data:
                return {
                    "tag_name": ble_device.name,
                    "battery_level": ble_device.battery_level,
                    "rssi": ble_device.coordinator_data.get("rssi"),
                    "hw_type": ble_device.coordinator_data.get("hw_type"),
                    "version": ble_device.coordinator_data.get("fw_version"),
