    ...
    async def get_tag_info(self, entity_id: str) -> Optional[tuple[TagType, str]]:
        ...
        try:
            ...
            if hasattr(hub, '_ble_hub') and hub._ble_hub is not None:
                # For BLE devices, get data directly from BLE hub
                ble_device = hub._ble_hub.get_device(tag_mac)
                if not ble_device:
                    raise HomeAssistantError(f"BLE device {tag_mac} not found")
                
                # Get hardware type from BLE device's advertising data
                if not ble_device._adv_data:
                    raise HomeAssistantError(
                        f"No advertising data available for BLE device {tag_mac}. "
                        "Please wait for the device to complete its next check-in."
                    )
                hw_type = ble_device._adv_data.hw_type
            else:
                # For AP devices, check if online and tag is registered
                if not hub.online:
                ...
                if tag_mac not in hub.tags:
                    raise HomeAssistantError(
                        ...
                        "If the tag has checke in, try restarting Home Assistant."
                    )
